---
# the default layout is 'page'
title: "О сайте"
icon: fas fa-info-circle
order: 5
---

![](/commons/1176361d2d6dc0e4bcc079459c07e6e8.jpg)

Давным-давно, когда деревья были большими, трава зеленее, да и я намного моложе, этот ресурс представлял из себя обычный блог, где постепенно накапливались статьи по администрированию **Linux** систем. Статей становилось больше, мои компетенции росли. Качество статей, а так же сложность рассматриваемых технологий и систем тоже увеличивалась. Блог развивался вместе со мной, становился все ближе к изначальной задумке - технической документации и базе знаний по **Linux** системам.

И вот, третьего дня, я решил все переосмыслить, и начал большую работу по модернизации сайта, параллельно переводя его с системы управления контентом **Wordpress** на генератор статических сайтов **Jekyll**.

**Wordpress** - отличная платформа для ведения блога, с тысячью тем оформления и миллионами плагинов. Но данный ресурс изначально был задуман как система документации, хранилищем моих наработок. Функционал **Wordpress** "из коробки" избыточен. В блоге было установлено минимум плагинов, я перепробовал большое количество тем оформления, в том числе платных.

Отличительной особенностью системы **Jekyll** является то, что сайт статичен, статьи генерируются шаблонизатором из **Markdown** файлов, а по-этому сайт очень шустро работает. Поиск по сайту моментальный. Все статьи хранятся в **GitHub** в формате **Markdown**. У сайта нет админки, нет хостинга, а значит не стоит задумываться о дырах в программном обеспечении или операционной системе на хостинге.

Благодаря работе по модернизации сайта, я так же ушел от излюбленных мною программ **Блокнот** и **Notepad++** к популярных нынче **Obsidian** и **VSCode**.

> Вы все так же можете [поддержать](https://yoomoney.ru/to/410011033838213) развитие этого сайта
{: .prompt-info }

Вся представленная документация хранится в репозитории [GitHub](https://github.com/bullvinkl/itdraft-docs.github.io). Если какие-то статьи у вас вызывают вопросы или сложности, вы можете добавлять [Issues](https://github.com/bullvinkl/itdraft-docs.github.io/issues), либо оставлять комментарии к соответствующим статьями. Я стараюсь отвечать на них, и при необходимости обновлять или вносить изменения в документацию.

# Дополнительное образование и повышение квалификации

Помимо изучения программного обеспечения, технологий и ведения документации, я периодически прохожу курсы повышения квалификации. Ниже представлен список курсов, который я успешно прошел, и применяю навыки в рабочем процессе. Этот список так же периодически пополняется.

## AL-1805. Безопасность в ОС Astra Linux Special Edition 1.8

> На курсе вы научитесь настраивать мандатный контроль доступа и мандатное управление целостностью для защиты конфиденциальных данных. Также вы освоите развертывание замкнутой программной среды для блокировки запуска недоверенного софта. Узнаете о метках безопасности и принципах работы ПО с ними для защиты конфиденциальных данных. Вы научитесь маркировать файлы и процессы, а также настраивать разграничение доступа.

![AL-1805](/assets/img/edu/al-1805-crt.png){: w="300" }
_Удостоверение о повышении квалификации_

![AL-1805](/assets/img/edu/al-1805-cv.png){: w="300" }
_Свидетельство_

## AL-1804. Сетевое Администрирование ОС Astra Linux Special Edition 1.8

> На курсе вы освоите полный спектр задач по управлению сетевыми службами: от базовой настройки TCP/IP до интеграции сложных инфраструктурных решений. Каждый модуль включает лабораторные работы.

![AL-1804](/assets/img/edu/al-1804-crt.png){: w="300" }
_Удостоверение о повышении квалификации_

![AL-1804](/assets/img/edu/al-1804-cv.png){: w="300" }
_Свидетельство_

## AL-1802. Администрирование Astra Linux Special Edition 1.8

> На курсе вы научитесь устанавливать операционную систему на различные устройства, администрировать их как по отдельности, так и в сети. Овладеете администрированием в удаленном режиме. Эти и другие умения вы отработаете на практике, решите различные задачи, включая устранения возникающих неполадок и сбоев.

![AL-1802](/assets/img/edu/al-1802-crt.png){: w="300" }
_Удостоверение о повышении квалификации_

![AL-1802](/assets/img/edu/al-1802-cv.png){: w="300" }
_Свидетельство_

## Docker для админов и разработчиков

> Освойте контейнеризацию для ускорения разработки, деплоя и сопровождения приложений. Программа выстроена от базовых команд до CI/CD и безопасности

![Docker для админов и разработчиков](/assets/img/edu/slurm-doc01.png){: w="300" }
_Свидетельство_

## Ansible: Infrastructure as Code

> Ключевые знания и навыки по Ansible. Принципы и подходы к Infrastructure as Code (IaC). Научиться писать плейбуки, создавать роли, управлять облачными ресурсами и обеспечивать безопасность конфигураций с Ansible Vault.

![Ansible: IaC](/assets/img/edu/slurm-ans01.png){: w="300" }
_Свидетельство_

## Безопасность проекта: аутентификация в Keycloak

> Курс помогает получить фундаментальные знания по работе с Keycloak и понять, как ускорить разработку и обеспечить безопасность проекта с минимальными усилиями.

![Keycloak](/assets/img/edu/slurm-kc01.png){: w="300" }
_Свидетельство_

## HR-11 Основы работы в Directum HR Pro 2.4

> Курс дает знания и навыки, необходимые для использования основных возможностей системы Directum HR Pro сотрудником кадровой службы, понимания правовых и методологических основ работы в Directum HR Pro

![HR-11](/assets/img/edu/hr-11-crt.png){: w="300" }
_Удостоверение о повышении квалификации_

![HR-11](/assets/img/edu/directum1.png){: w="300" }
_Сертификат_

![HR-11](/assets/img/edu/hr-11.png){: w="300" }
_Протокол результатов аттестации_

## RX-42 Установка и администрирование Directum RX 4.12 на Linux

> Курс ориентирован на специалистов, планирующих заниматься установкой, администрированием, сопровождением системы Directum RX на операционной системе Linux.

![RX-42](/assets/img/edu/rx-42-2.png){: w="300" }
_Удостоверение о повышении квалификации_

![RX-42](/assets/img/edu/rx-42.png){: w="300" }
_Протокол результатов аттестации_

## DevOps. Уровень 2. Использование Kubernetes

> Цель курса – научиться устанавливать и администрировать кластер kubernetes, разворачивать и настраивать в нём приложения. Вы изучите полный процесс CI/CD в кластере kubernetes от сборки до деплоя.

![DevOps. Уровень 2](/assets/img/edu/do2-crt.png){: w="300" }
_Удостоверение о повышении квалификации_

![DevOps. Уровень 2](/assets/img/edu/do2-crt-r.png){: w="300" }
_Сертификат_

![DevOps. Уровень 2](/assets/img/edu/do2-crt-e.png){: w="300" }
_Certificate of Achievement_

## DevOps. Уровень 1. Инфраструктура как код, основные инструменты

> Данный курс разработан специально для инженеров DevOps и системных администраторов, которые хотят освоить IaC. Учитывая, что сейчас идут процессы трансформации профессии системного администратора Linux в инженера DevOps-технологий, обучение будет полезно для технических специалистов, работающих с Linux.

![DevOps. Уровень 1](/assets/img/edu/do1-crt.png){: w="300" }
_Удостоверение о повышении квалификации_

![DevOps. Уровень 1](/assets/img/edu/do1-crt-r.png){: w="300" }
_Сертификат_

![DevOps. Уровень 1](/assets/img/edu/do1-crt-e.png){: w="300" }
_Certificate of Achievement_

## Linux. Уровень 3. Обеспечение безопасности систем, сервисов и сетей

> Курс позволяет получить ключевые знания по обеспечению комплексной безопасности сетевой инфраструктуры средствами UNIX (Linux) мирового уровня. Порядка 70% курса посвящается практике по усилению защиты сети, включающей в себя построение защищенного внешнего периметра средствами демилитаризованной зоны на основе Linux, а в качестве рабочих станций применяются Windows и Linux.

![Linux. Уровень 3](/assets/img/edu/lin3-crt.png){: w="300" }
_Удостоверение о повышении квалификации_

![Linux. Уровень 3](/assets/img/edu/lin3-crt-r.png){: w="300" }
_Сертификат_

![Linux. Уровень 3](/assets/img/edu/lin3-crt-e.png){: w="300" }
_Certificate of Achievement_

## Linux. Уровень 7. Управление сетью на основе методологии NetDevOps

> В этом курсе будет рассмотрены различные варианты организации отказоустойчивого ядра сети и мониторинг/управление конфигурацией устройств, с использование последних достижения в этой области.
> Приятным бонусом курса будет знакомство с оборудованием компании Cisco. Практический опыт совместной эксплуатации Cisco и Linux систем доказал, что они прекрасно дополняют друг друга, и это не случайно – Cisco и Linux имеют очень много общего.
> Данный курс предназначен для системных администраторов, работающих с системами Linux и использующих или планирующих использовать коммутаторы и маршрутизаторы Cisco в своих сетях.

![Linux. Уровень 7](/assets/img/edu/lin7-crt.png){: w="300" }
_Удостоверение о повышении квалификации_

![Linux. Уровень 7](/assets/img/edu/lin7-crt-r.png){: w="300" }
_Сертификат_

![Linux. Уровень 7](/assets/img/edu/lin7-crt-e.png){: w="300" }
_Certificate of Achievement_

## DBA3. Администрирование PostgreSQL. Резервное копирование и репликация

> PostgreSQL 16|13 - одна из самых распространенных СУБД. Это открытая, технологичная система управления базами данных, поддерживающая широкий спектр возможностей стандарта SQL.
> Вы научитесь
> - выполнение резервного копирования различными способами
> - настройка серверов для физической и логической репликации
> - знакомство со сценариями использования репликации
> - представление о способах построения кластеров

![DBA3](/assets/img/edu/dba3-crt.png){: w="300" }
_Удостоверение о повышении квалификации_

![DBA3](/assets/img/edu/dba3-crt-r.png){: w="300" }
_Сертификат_

![DBA3](/assets/img/edu/dba3-crt-e.png){: w="300" }
_Certificate of Achievement_

![DBA3](/assets/img/edu/dba3-crt-pg.jpg){: w="300" }
_Сертификат Postgres PRO_

## DBA2. Администрирование PostgreSQL. Настройка и мониторинг

> PostgreSQL – свободная кроссплатформенная объектно-реляционная система управления базами данных. Ориентирована на широкий круг решаемых задач. Входит в число лидеров среди реляционных СУБД. Цель курса – изучить устройство PostgreSQL и важнейшие приемы ее администрирования.
> Вы научитесь:
> - настраивать различные конфигурационные параметры исходя из понимания внутренней организации сервера,
> - мониторить сервер с использованием обратной связи для итеративной настройки параметров,
> - настраивать параметры, связанные с локализацией,
> - управлять расширениями и получит представление о процедуре обновления сервера.

![DBA2](/assets/img/edu/dba2-crt.png){: w="300" }
_Удостоверение о повышении квалификации_

![DBA2](/assets/img/edu/dba2-crt-r.png){: w="300" }
_Сертификат_

![DBA2](/assets/img/edu/dba2-crt-e.png){: w="300" }
_Certificate of Achievement_

![DBA2](/assets/img/edu/dba2-crt-pg.jpg){: w="300" }
_Сертификат Postgres PRO_

![DBA2](/assets/img/edu/dba2-crt-vr.png){: w="300" }
_Сертификат: Расширенный курс по администрированию PostgreSQL_

## DBA1. Администрирование PostgreSQL. Базовый курс

> Postgres Pro - Российская СУБД, разработанная компанией Postgres Professional на основе свободно-распространяемой СУБД PostgreSQL. Postgres Pro входит в реестр российского ПО. Цель курса - дать слушателям общее представление о PostgreSQL и его архитектуре.
> Вы научитесь
> - разбираться в архитектуре PostgreSQL,
> - устанавливать, проводить базовую настройку, управлять сервером,
> - организовывать данные на логическом и физическом уровнях,
> - решать базовые задачи администрирования,
> - управлять пользователями и доступом,
> - получать представление о резервном копировании и репликации.

![DBA1](/assets/img/edu/dba1-crt.png){: w="300" }
_Удостоверение о повышении квалификации_

![DBA1](/assets/img/edu/dba1-crt-r.png){: w="300" }
_Сертификат_

![DBA1](/assets/img/edu/dba1-crt-e.png){: w="300" }
_Certificate of Achievement_

![DBA1](/assets/img/edu/dba1-crt-pg.jpg){: w="300" }
_Сертификат Postgres PRO_

## Программирование на языке Python. Уровень 1. Базовый курс

> На курсе вы освоите основы Python: изучите синтаксис языка, получите базовые навыки структурного и процедурного программирования, познакомитесь со структурами данных языка Пайтон.
> Вы научитесь устанавливать и настраивать среду разработки, применять базовые конструкции Python, создавать модули и пакеты, пользоваться основными структурами данных, выполнять основные операции ввода/вывода.
> Также вы получите необходимую подготовку для изучения объектно-ориентированного программирования на языке Python.

![Python. Уровень 1](/assets/img/edu/py1-crt.png){: w="300" }
_Удостоверение о повышении квалификации_

![Python. Уровень 1](/assets/img/edu/py1-crt-r.png){: w="300" }
_Сертификат_

![Python. Уровень 1](/assets/img/edu/py1-crt-e.png){: w="300" }
_Certificate of Achievement_

## ADN-021. Практика работы с Kubernetes. Базовые темы

> В данном курсе вы получите все необходимые знания для работы с Kubernetes. Вы узнаете, что такое Kubernetes, познакомитесь с его архитектурой, изучите основные компоненты и службы, чтобы запустить ваш проект. Вы поднимите учебный кластер Kubernetes, опишите и запустите такие сущности Kubernetes, как поды, наборы реплик и развёртывания. Вы узнаете, как организовано сетевое взаимодействие в Kubernetes, какие имеются виды сервисов, и когда их лучше использовать. Вы настроите приложение с помощью ConfigMaps и будете использовать сущности Secrets для хранения конфиденциальных данных. Курс заканчивается обсуждением томов для долговременного хранения данных. Помимо фундаментальных знаний, вы узнаете устранять неполадки в случае, если что-то пойдет не так.

![ADN-021](/assets/img/edu/adm021-crt.png){: w="300" }
_Удостоверение о повышении квалификации_

## Linux. Уровень 6. Создание отказоустойчивых кластерных решений

> На этом курсе Вы научитесь создавать высоконадежные решения, на основе свободных программных продуктов в операционной системой Linux. Вы сможете плавно мигрировать подключение Вашей сети к Интернет в отказоустойчивую конфигурацию с несколькими шлюзами и распределять нагрузку между несколькими провайдерами. Познакомитесь с решениями Pacemaker/Corosync, позволяющими собрать отказоустойчивую конфигурацию для любых сервисов – HTTP, iSCSI, CIFS, SMTP/IMAP, IP телефонии и прочих, узнаете, как использовать контейнерную (LXC) и аппаратную (KVM) виртуализацию в отказоустойчивой среде.

![Linux. Уровень 6](/assets/img/edu/lin6-crt.png){: w="300" }
_Удостоверение о повышении квалификации_

![Linux. Уровень 6](/assets/img/edu/lin6-crt-r.png){: w="300" }
_Сертификат_

![Linux. Уровень 6](/assets/img/edu/lin6-crt-e.png){: w="300" }
_Certificate of Achievement_

## Администратор Naumen Contact Center

> Программа курса рассчитана на технических специалистов системы Naumen Contact Center. В рамках обучения рассматриваются следующие темы:
> - архитектура решения;
> - подключение к оператору связи;
> - централизованная настройка программных телефонов;
> - управление звуковыми файлами и настройка ivr-скриптов в графическом редакторе IVRBuilder;
> - методы диагностики и решения проблем.

![Naumen Contact Center](/assets/img/edu/naucc-crt.png){: w="300" }
_Сертификат_

# Конференции:

## PG BootCamp 2026

![PG BootCamp Russia 2026 Moscow](/assets/img/edu/pgbc-moscow-2026.png){: w="300" }
_PG BootCamp Russia 2026 в г.Москва_

## PG BootCamp 2024

![PG BootCamp Russia 2024 Kazan](/assets/img/edu/pgbc-kazan-2024.png){: w="300" }
_PG BootCamp Russia 2024 в г.Казань_

![PG BootCamp 2024 Minsk](/assets/img/edu/pgbc-minsk-2024.png){: w="300" }
_PG BootCamp 2024 в г.Минск_
